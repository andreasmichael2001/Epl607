...
def load_obj_model(file_path, angle_y):
...
def render_frame(angle, frame_id):
    image = Image.new("RGB", (WIDTH, HEIGHT), (0, 0, 0))  # black background
    model_data = load_obj_model("bunny.obj", angle)
    material = Material([80, 130, 255], [140, 140, 140], 16, "both")  # blue bunny
    light = Light([0.3, 1.5, 3.5], np.array([600, 600, 600]))
    fill = Light([0, -1.5, 4.5], np.array([180, 180, 180]))
    ...
    for tri, norms in model_data:
        ...
        colors = []
        for v, n in zip(tri, norms):
            color = phong_shading(v, n, material, light, view_pos) + phong_shading(v, n, material, fill, view_pos)
            colors.append(color)
    ...
    image.save(f"{OUTPUT_DIR}/frame_{frame_id:03d}.png")

if __name__ == "__main__":
    for frame in range(FRAMES):
        angle = frame * (360 / FRAMES)
        print(f"Rendering frame {frame+1}/{FRAMES} (angle={angle:.2f})")
        render_frame(angle, frame)
    print("All frames rendered. Use ffmpeg to create a video:")
    print("ffmpeg -framerate 30 -i frames/frame_%03d.png -c:v libx264 -pix_fmt yuv420p output1.mp4")
